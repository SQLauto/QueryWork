/*
	...
	Note: the DDL actions are logged to WilcoR2.Utlity.dbo.Sim_ObjectHistory using a stored procedure.
	So, from some servers you will have to modify the script with a linked server.

	$Workfile: Drop_DuplicateObjects.sql $
	$Archive: /SQL/QueryWork/Drop_DuplicateObjects.sql $
...
Begin	-- Process Cursor data
	...
	Begin
		--print @DDL_Op;
		EXEC Utility.dbo.Sim_ObjectHistoryInsert @DBName = @DBName, @ObjName = @ObjName
...
Return
/*
Select * From Utility.dbo.Sim_ObjectHistory

--Truncate Table Utility.dbo.Sim_ObjectHistory;
...
Select distinct Type, type_desc from sys.objects as o
